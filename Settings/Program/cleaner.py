...
def rich_presence():
    try:
        rpc = Presence("1281925527229169724")
        rpc.connect()
        rpc.update(
            large_image= "icon",
            large_text = f"https://discord.gg/bluetiger",
            details = f"Cleaner",
            state = "Cleaner BlueTiger",
            buttons=[{"label": "Discord", "url": "https://discord.gg/bluetiger"}]
        )
...
░                                                                                  
 ...
 [1] Scan & clean       [4] Analyse one file               
 [2] Scan only          [5] Analyse one directory            
 [3] Clean only         [6] Exit
    """
    fadedmenu = fade.water(menu)
